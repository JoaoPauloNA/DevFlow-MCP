...
def inspect_page(root):
    ...
    # One non-redundant real render is the minimum evidence for a small QA
    # report. Reuse the exact capture for the paired White-Box report rather
    # than creating decorative duplicate screenshots.
    for name, size in (('desktop',(1536,1024)),):
        shot = out / f'{name}.png'
        if shot.is_file() and shot.stat().st_size >= 1000:
            result['screenshots'].append(str(shot)); continue
        profile = out / f'.browser-{name}'
        cmd = [BRAVE,'--headless=new','--disable-gpu','--hide-scrollbars','--timeout=3000',f'--user-data-dir={profile}',f'--window-size={size[0]},{size[1]}',f'--screenshot={shot}',html.as_uri()]
        try:
            ...
            # Chromium can keep helper processes alive after writing the
            # screenshot. The bytes, not a graceful browser exit, are the
            # evidence gate; never fabricate an image on timeout.
            time.sleep(1)
            if shot.is_file() and shot.stat().st_size >= 1000: result['screenshots'].append(str(shot))
            else: result['issues'].append(f'{name} screenshot timed out')
