...
class Worker:
    ...
    def engineer(self,w):
        if not w['vault_path']:
            ...
            self.db.event(w['id'],None,'WARN','[VAULT] path required'); return
        try: qa_root(w['vault_path'])
        except Exception as e:
            self.db.setwf(w['id'],status='NEEDS_USER',blocker='Não sei qual é o vault correto deste projeto. Informe o caminho.')
            self.db.event(w['id'],None,'WARN','[VAULT] QA directory ambiguous',{'reason':str(e)}); return
    ...
    def qa(self,w,t,role='QA'):
        ...
        if verdict=='PASS':
            ...
            return
        if verdict=='BLOCKED':
            self.db.setwf(w['id'],status='BLOCKED',blocker=f'{role} blocked: {report}',last_result=verdict); return
        n=t['corrections']+1
    ...
    def step(self,wid):
        ...
        try:
            ...
            ts=self.db.tasks(wid); active=next((x for x in ts if x['status'] in ('PENDING','DEV','QA','BLACK_BOX_QA','WHITE_BOX_QA','INDEPENDENT_QA','FINAL_ENGINEERING')),None)
            if not active:
    ...
    def run_forever(self):
        while True:
            with self.db.con() as c: ids=[r['id'] for r in c.execute("SELECT id FROM workflows WHERE status IN ('ENGINEERING','DEV','QA','BLACK_BOX_QA','WHITE_BOX_QA','INDEPENDENT_QA','FINAL_ENGINEERING') AND paused=0")]
